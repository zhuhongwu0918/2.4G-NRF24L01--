...
HAL_StatusTypeDef HAL_nRF24L01P_Init(nRF24L01P *nRF)
{
	...
	HAL_nRF24L01P_CE_High(nRF);//zhhw
...
HAL_StatusTypeDef HAL_nRF24L01P_IRQ_Handler(nRF24L01P *nRF)
{
	...
	if((regStatus & (1 << 6)) != 0)
	{
		...
		do {
			...
			uint8_t Zero = 0xff;
			uint8_t  RX_PD_WD;
			HAL_nRF24L01P_SendCommand(nRF, 0x60, &Zero,&RX_PD_WD,1);//HAL_nRF24L01P_WriteRegister//HAL_nRF24L01P_SendCommand(nRF, nRF_CMD_W_REGISTER + regAddr, pRegData, &Zero, 1)
			nRF->PayloadWidth = RX_PD_WD;
			//**********//
